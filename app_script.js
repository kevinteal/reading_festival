// JavaScript Document
var db = openDatabase('reading_2015_bands_db', '1.0', 'bands db', 2 * 1024 * 1024);
var total_stages =0;
var day_arr=[];
var day_name_arr=[];
var band_db_table = "bands_101";
$(document).ready(function(e) {
    ...
    $('#popupInfo_mainpage, #popupInfo_fav').popup('reposition', 'positionTo: window');
});
});
...
function add_to_plan2(e,method){
	...
	if(val_text == "on"){
		...
		db.transaction(function (txs) {
				txs.executeSql('UPDATE '+band_db_table+' SET band_fav=1 WHERE id='+band_id+'');
		});
		...
		db.transaction(function (txs) {
				txs.executeSql('UPDATE '+band_db_table+' SET band_fav=0 WHERE id='+band_id+'');
		});
	}
...
function set_up_stages(){
	db.transaction(function (txs) {
						txs.executeSql('select max(stage_rank) as total_stages from '+band_db_table+' ', [], function(txs, results){
...
function set_up_main_page(){
	...
	db.transaction(function (txs) {
		...
		//how many stages?
		txs.executeSql('select DISTINCT stage_rank, stage from '+band_db_table+' order by stage_rank ASC', [], function(txs, results){
						...
						txs.executeSql('select day,day_name from '+band_db_table+' group by day order by day asc', [], function(txs, results){
							var len = results.rows.length, i;
							for(i=0;i<len;i++)
								{	
								...
								if( (day_arr.indexOf(parseInt(fulldate))!=-1) || (fulldate == extra_day)){
									...
									if(time<700){
	 								   ...
	 								   sql = "select * from "+band_db_table+" where day = "+sqlfulldate+" and start_time > 700 and finish_time BETWEEN "+time+" AND 700";
									}else{
									sql = "select * from "+band_db_table+" where day = "+sqlfulldate+" and start_time <= "+time+" and finish_time > "+time;
									}
									...
									if(time>700 && len == 0 ){
										db.transaction(function (txs2) {
											sql = "select * from "+band_db_table+" where day = "+sqlfulldate+" and start_time BETWEEN 700 AND "+time+" and finish_time BETWEEN 000 AND 700";
											txs2.executeSql(sql, [], function(txs, results){
												var len = results.rows.length, i;
												console.log(len);
												for(i=0;i<len;i++)
...
//if time is greater than 23 AND NO RESULTS FOUND look for day BEFORE
	...
	if(time<700){
			sqlfulldate=sqlfulldate-1;
			sql = "select * from "+band_db_table+" where day = "+sqlfulldate+" and start_time > "+time+" and stage_rank='"+StageName+"' order by start_time ASC Limit 1";
	}else{
		sql = "select * from "+band_db_table+" where day = "+fulldate+" and start_time > "+time+" and stage_rank='"+StageName+"' order by start_time ASC Limit 1";
	}
	...
	txs.executeSql(sql, [], function(txs, results){
							...
							if(time>700 && len == 0 ){
								var sqlfulldate2 = fulldate - 1;
									db.transaction(function (txs2) {
										sql = "select * from "+band_db_table+" where day = "+fulldate+" and start_time BETWEEN 000 AND 700 and stage_rank='"+StageName+"' order by start_time ASC Limit 1 ";
										txs2.executeSql(sql, [], function(txs, results){
											var len = results.rows.length, i;
											//console.log(len);
											for(i=0;i<len;i++)
...
if(stageNum!="AZ"){
 db.transaction(function (txs) {
						...
						txs.executeSql('select * from '+band_db_table+' where stage_rank = '+stageNum+' and start_time>=700 order by start_time DESC ', [], function(txs, results){
							var len = results.rows.length, i;
							for(var i=0;i<len;i++)
								{	
						...
						txs.executeSql('select * from '+band_db_table+' where stage_rank = '+stageNum+' and start_time<700 order by start_time ASC ', [], function(txs, results){
	...
	//user has selected AZ of all bands.
	console.log("user clicked AZ");
	//get earlist day of festival. put all bands inside one tab
	var az_day = "";
	db.transaction(function (txs) {
		 	
			txs.executeSql('select day from '+band_db_table+' order by day ASC Limit 1', [], function(txs, results){
			...
			txs.executeSql('select * from '+band_db_table+' order by band_name ASC', [], function(txs, results){
				var len = results.rows.length, i;
				for(var i=0;i<len;i++)
					{	
						var BandRecord = results.rows.item(i);
						var content = lineup_content_az(BandRecord);	
						$("#tab_"+az_day).append(content);	
						console.log(BandRecord.band_name);	
					}
					console.log(az_day);
		$(".lineup_band_az").trigger('create');
	 ...
	 db.transaction(function (txs) {
		// console.log(day_arr);
		 $.each(day_arr, function (index,val){
						...
						txs.executeSql('select * from '+band_db_table+' where band_fav=1 and day='+val+' and start_time>=700 order by start_time ASC ', [], function(txs, results){
							var len = results.rows.length, i;
...
function clearplan(){
	...
	db.transaction(function (txs) {
				txs.executeSql('UPDATE '+band_db_table+' SET band_fav=0 WHERE band_fav=1');
		});
...
function lineup_content_az(BandRecord){
	$("#stage_name_heading").text("AZ Bands");
	var finish_time = BandRecord.finish_time.toString();
	if(finish_time.length<4){
		finish_time=add_zeros(finish_time);
	}						
	...
	if(start_time.length<4){
		...
		if(BandRecord.band_fav==1){
			...
			if(name_length>20){
				...
				var content = '<div class="lineup_band_az">'+
								'<div class="az_band_name">'+show_name+'</div>'+
								'<div class="az_time_stage"><span class="darker_text small_text">'+BandRecord.stage.substring(0,10)+'...</span></div>'+
								'<div class="az_time_stage"><span class="darker_text small_text">'+start_time+ ' - ' +finish_time+'</span></div>'+
								'<div class="az_day_switch"><span class="darker_text small_text">'+BandRecord.day_name.substring(0,3)+'</span></div>'+
								'<div class="az_day_switch">'+
								'<form><select name=flip'+BandRecord.id+' id=flip'+BandRecord.id+' data-role="flipswitch" data-mini="true" data-theme="c" onChange="add_to_plan2(this,0)">'+
						'<option value="off" '+flip_off+' >Off</option> <option value="on" '+flip_on+' >On</option></select></form> </div>'
								'</div>';
